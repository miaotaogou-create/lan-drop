...
- 聊天记录落盘到配置目录 `chat.json`，每端最多 500 条；进行中的收/发文件卡不落盘；对端右键可清空该会话并立即写盘
- 收文件时 HttpServer 节流发出进度，聊天区显示「接收中 N%」，完成后变为可打开卡片
- 文件夹：无嵌套时发顶层普通文件；有嵌套时可选本机 `tar` 打 zip 后走 `/api/upload`，或仍「仅发顶层」。对端收到 zip 文件，不自动解压。无整树流式协议。
- 抖动：inbox 文本为 `__landrop_nudge__` 时触发窗口抖动，不进普通聊天语义
- 同一网段互信，接口没有鉴权
